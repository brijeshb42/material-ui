...
import { generateSxCss, getTheme, type ICustomOptions, type SxConfig } from './common';
...
> extends BaseProcessor {
  ...
  protected get tagExpressionArguments(): Expression[] {
    const overallExp: Expression[] = [];

    const t = this.astService;
    if (typeof this.component === 'string') {
      if (this.component === 'FunctionalComponent') {
        overallExp.push(t.arrowFunctionExpression([], t.blockStatement([])));
      } else {
        overallExp.push(singleQuotedStringLiteral(this.component));
      }
    } else if (!this.component) {
      throw new Error('Could not identify component name');
    } else {
      overallExp.push(t.callExpression(t.identifier(this.component.node.name), []));
    }

    const [, firstArgParams] = this.params;
    if (firstArgParams[0] === 'call' && firstArgParams.length === 3) {
      const [, , metaArg] = firstArgParams;
      if (metaArg.kind === ValueType.LAZY) {
        overallExp.push(t.callExpression(t.identifier(metaArg.ex.name), []));
      } else if (metaArg.kind === ValueType.CONST) {
        overallExp.push(metaArg.ex);
      }
    }
  ...
  protected get tagExpression(): CallExpression {
    const t = this.astService;
    return t.callExpression(this.callee, this.tagExpressionArguments);
  }
