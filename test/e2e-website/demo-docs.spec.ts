...
const DOCS_INFRA_PATH = '/experiments/docs/docs-infra/';
...
export default function BasicButtons() {
  ...
  // The demos on this page opt into docs-infra with `"docsInfra": true`: their
  // source is loaded, resolved, and executed by docs-infra rather than by the
  // legacy pipeline and `ReactRunner`. Everything the reader touches — the
  // toolbar, the editor, expansion, reset — is still Material's, so the editor
  // re-highlights with Prism as it does for any other demo.
  test.describe('docs-infra demos', () => {
    test.beforeEach(async ({ page }) => {
      await page.goto(DOCS_INFRA_PATH);
    });

    test('opens on the preview region docs-infra resolved', async ({ page }) => {
      const demo = getDemo(page, 'DemoInDocsDocsInfra', 1);

      // docs-infra's projection is a region of the file, so unlike a
      // `.tsx.preview` file it keeps the newline that ends that region.
      await expect(demo.locator('textarea')).toHaveValue(
...
`,
    ...
    test('runs an edited preview region in place', async ({ page }) => {
      const demo = getDemo(page, 'DemoInDocsDocsInfra', 1);

      await demo.locator('textarea').fill(`<Stack sx={{ width: '100%' }} spacing={2}>
  <Alert severity="success">Edited live</Alert>
</Stack>`);

      // The fragment is patched back into the whole file before it runs, so its
      // imports are still there.
      await expect(demo.getByRole('alert').filter({ hasText: 'Edited live' })).toBeVisible();
    });

    test('discards a preview edit when expanding', async ({ page }) => {
      const demo = getDemo(page, 'DemoInDocsDocsInfra', 1);
      const editor = demo.locator('textarea');

      await editor.fill(`<Stack sx={{ width: '100%' }} spacing={2}>
  <Alert severity="info">Preview edit</Alert>
</Stack>`);
      await expect(demo.getByRole('alert').filter({ hasText: 'Preview edit' })).toBeVisible();

      await demo.getByRole('button', { name: 'Expand code' }).click();

      // Crossing the collapse boundary discards the edit: a region edit is not
      // meaningful against the whole file.
      await expect(editor).not.toHaveValue(/Preview edit/);
      await expect(editor).toHaveValue(/export default function DemoInDocsDocsInfra/);
    });

    test('reports an error from docs-infra and recovers on reset', async ({ page }) => {
      const demo = getDemo(page, 'DemoInDocsDocsInfra', 1);
      await demo.getByRole('button', { name: 'Expand code' }).click();
      const editor = demo.locator('textarea');

      await editor.fill("throw new Error('Runtime crash');");

      // The error comes from docs-infra's runner rather than `ReactRunner`.
